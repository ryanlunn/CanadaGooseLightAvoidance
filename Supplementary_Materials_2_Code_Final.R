...
OBJ<-read.csv("Clear.12pm Sky and LEDs.csv", header=TRUE)
...
irr_cloud<-read.csv("Cloudy.12pm.csv", header=TRUE)# opening the ambient light/irradiance spectrum file
irradiance_cloud<-as.rspec(irr_cloud)  # converts to rspec object for pavo

##Ocular Media Transmittance curve
cago_om<-read.csv("CAGO_OM_vector.csv", header=T)
head(cago_om)
...
wl<-seq(300,700,by=1)
ggplot(data=NULL, aes(x = wl , y = OBJ$x550, colour=wl)) + 
  geom_line(lwd=2)+
  ylim(c(0,4000))+
  scale_color_wavelength() + 
  ylab("Photon(counts)")+
...
spec_data<-read.csv("Single Choice Lights_Processed Spectra.csv")
...
man_data<-read.csv("Manipulative_Experiment.csv")
head(man_data)
man_data$Notes<-NULL
...
bird_data <- read.csv("CAGO_Data.csv", na.strings = c("","NA"), header=TRUE) #you can save a normal Excel file as a .csv using "Save As" in excel
